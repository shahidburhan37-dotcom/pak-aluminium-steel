...
const gallerySections = categories.map(c => ({
  ...c,
  items: getProductsByCategory(c.slug).slice(0, 12),
}))
  ...
  return (
    <div className="carousel-wrapper">
      <div className="carousel-track" ref={trackRef} onScroll={handleScroll}>
        {data.items.map((item) => (
          <Link to={`/product/${item.slug}`} key={item.slug} className="gallery-card" style={{ textDecoration: 'none' }}>
      ...
      <div className="carousel-dots">
        {Array.from({ length: Math.min(totalDots, 8) }, (_, i) => (
          <button
            key={i}
      ...
      <div className="page-header" style={{ paddingTop: 'calc(70px + 15px)' }}>
        ...
        <div className="filter-tabs animate-hero-up delay-2">
          {gallerySections.map(tab => (
            <button key={tab.id} className="filter-tab" onClick={() => scrollToSection(tab.slug)}>
              {tab.title.split(' ')[0]}
            </button>
          ))}
        </div>
