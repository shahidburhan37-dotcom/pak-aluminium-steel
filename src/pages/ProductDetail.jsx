      ...
      <section style={{ position: 'relative', height: '60vh', minHeight: 400, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: 20, left: 0, right: 0, padding: '0 clamp(20px, 5vw, 52px)', zIndex: 10 }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <svg width="12" height="12" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg>
            <span style={{ color: 'var(--color-gold)', fontSize: 13, fontWeight: 600 }}>{product.name}</span>
          </div>
        </div>
      ...
      <section id="whatsapp" style={{ background: '#f7f8f5', padding: 'clamp(48px, 6vw, 72px) clamp(20px, 5vw, 52px)', maxWidth: '100%', borderRadius: 24, margin: '0 auto 12px', width: 'calc(100% - 24px)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <RevealDiv type="scale">
            ...
            <WhatsAppButton productName={product.name} style={{ fontSize: 17, padding: '18px 40px', borderRadius: 14 }} />
          </RevealDiv>
        </div>
      </section>

      {/* More Products */}
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(20px, 5vw, 52px) 48px' }}>
        <RevealDiv type="up">
          <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 24, marginBottom: 24 }}>More Products</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
            {relatedProducts.map(rp => (
              <Link to={`/product/${rp.slug}`} key={rp.slug} style={{ textDecoration: 'none', borderRadius: 16, overflow: 'hidden', background: 'var(--color-surface)', border: '1px solid var(--color-border)', transition: 'all 0.3s' }}>
                <img src={rp.img} alt={rp.name} style={{ width: '100%', height: 200, objectFit: 'cover' }} loading="lazy" />
                <div style={{ padding: 16 }}>
                  <div style={{ fontSize: 12, color: 'var(--color-accent)', fontWeight: 600, marginBottom: 4 }}>{rp.category}</div>
                  <div style={{ fontFamily: 'var(--font-heading)', fontSize: 16, fontWeight: 700, color: 'var(--color-text)' }}>{rp.name}</div>
                  <div style={{ fontSize: 13, color: 'var(--color-text-muted)', marginTop: 4 }}>{rp.tag}</div>
                </div>
              </Link>
            ))}
          </div>
